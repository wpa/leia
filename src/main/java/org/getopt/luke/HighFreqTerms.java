...
/**
 * <b>NOTE: this is a temporary copy of contrib/misc from Lucene - we can get rid of it when LUCENE-2638 is
 * integrated.</b> <code>HighFreqTerms</code> class extracts the top n most frequent terms (by document frequency ) from
 * an existing Lucene index and reports their document frequencey. If the -t flag is and reports both their document
 * frequency and their total tf (total number of occurences) in order of highest total tf
 */
public class HighFreqTerms {
  ...
  public static void main(final String[] args) throws Exception {
    ...
    final String[] fields = field != null ? new String[]{field} : null;
    reader = DirectoryReader.open(dir);
    final TermStats[] terms = getHighFreqTerms(reader, numTerms, fields);
    if (!IncludeTermFreqs) {
      ...
      final TermStats[] termsWithTF = sortByTotalTermFreq(reader, terms);
   ...
   * @throws Exception
   */
  public static TermStats[] getHighFreqTerms(final IndexReader reader, final int numTerms, final String[] fieldNames) throws Exception {
  ...
  /**
   * Takes array of TermStats. For each term looks up the tf for each doc containing the term and stores the total in
   * the output array of TermStats. Output array is sorted by highest total tf.
   * 
   * @param reader
  ...
  public static TermStats[] sortByTotalTermFreq(final IndexReader reader, final TermStats[] terms) throws Exception {
    final TermStats[] ts = new TermStats[terms.length]; // array for sorting
    long totalTF;
    for (int i = 0; i < terms.length; i++) {
    ...
    final Comparator<TermStats> c = new TotalTermFreqComparatorSortDescending();
    Arrays.sort(ts, c);
...
/**
 * Comparator Reverse of normal Comparator. i.e. returns 1 if a.totalTermFreq is less than b.totalTermFreq So we can
 * sort in descending order of totalTermFreq
 */
